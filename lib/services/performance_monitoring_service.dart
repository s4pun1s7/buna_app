...
class PerformanceMonitoringService {
  ...
  void _collectMemoryMetrics() async {
    try {
      ...
      if (memoryInfo > 200.0) { // MB
        if (kDebugMode) {
          print('⚠️ High memory usage detected: ${memoryInfo.toStringAsFixed(1)}MB');
        }
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<double> _getMemoryUsage() async {
    ...
    return _memoryUsageHistory.isNotEmpty 
        ? _memoryUsageHistory.last + (DateTime.now().millisecondsSinceEpoch % 10 - 5)
  ...
  void trackRouteNavigationEnd(String routeName) {
    ...
    if (startTime != null) {
      ...
      if (kDebugMode) {
        print('✅ Navigation to $routeName completed in ${duration.inMilliseconds}ms');
      }
      
      // Alert for slow navigation (> 2 seconds)
      if (duration.inMilliseconds > 2000) {
        if (kDebugMode) {
          print('⚠️ Slow navigation detected for $routeName: ${duration.inMilliseconds}ms');
        }
      }
    }
  }
  ...
  void trackImageLoadEnd(String imagePath) {
    ...
    if (startTime != null) {
      ...
      if (duration.inMilliseconds > 1000) {
        if (kDebugMode) {
          print('⚠️ Slow image loading: $imagePath took ${duration.inMilliseconds}ms');
        }
      }
    }
  }
  ...
  Map<String, dynamic> getPerformanceMetrics() {
    final avgNavigationTime = _routeNavigationTimes.values.isNotEmpty
        ? _routeNavigationTimes.values.map((d) => d.inMilliseconds).reduce((a, b) => a + b) / _routeNavigationTimes.length
        : 0.0;

    final avgImageLoadTime = _imageLoadTimes.values.isNotEmpty
        ? _imageLoadTimes.values.map((d) => d.inMilliseconds).reduce((a, b) => a + b) / _imageLoadTimes.length
        : 0.0;

    final currentMemory = _memoryUsageHistory.isNotEmpty ? _memoryUsageHistory.last : 0.0;
    ...
    return {
      'navigation': {
        ...
        'slow_navigations': _routeNavigationTimes.values.where((d) => d.inMilliseconds > 2000).length,
      },
      'images': {
        'average_load_time_ms': avgImageLoadTime,
        'total_loads': _imageLoadTimes.length,
        'slow_loads': _imageLoadTimes.values.where((d) => d.inMilliseconds > 1000).length,
        'most_loaded': _getMostLoadedImage(),
      },
      'memory': {
        'current_mb': currentMemory,
        'average_mb': avgMemory,
        'peak_mb': _memoryUsageHistory.isNotEmpty ? _memoryUsageHistory.reduce((a, b) => a > b ? a : b) : 0.0,
      },
    };
  }
...
${_generateRecommendations(metrics)}
  ...
  String _generateRecommendations(Map<String, dynamic> metrics) {
    ...
    if (metrics['navigation']['slow_navigations'] > 0) {
      recommendations.add('• Consider implementing lazy loading for slow routes');
    }
    
    if (metrics['images']['slow_loads'] > 0) {
      recommendations.add('• Optimize large images or implement progressive loading');
    }
    
    if (metrics['memory']['peak_mb'] > 150.0) {
      recommendations.add('• Monitor memory usage - consider implementing image cache cleanup');
    }
    
    if (recommendations.isEmpty) {
      recommendations.add('• Performance looks good! Keep monitoring for regressions');
    }
