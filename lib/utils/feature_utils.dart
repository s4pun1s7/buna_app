...
class FeatureUtils {
  ...
  static bool get allFestivalFeaturesEnabled => FeatureFlags.allFestivalFeaturesEnabled;

  /// Check if all interactive features are enabled
  static bool get allInteractiveFeaturesEnabled => FeatureFlags.allInteractiveFeaturesEnabled;

  /// Check if all support features are enabled
  static bool get allSupportFeaturesEnabled => FeatureFlags.allSupportFeaturesEnabled;
  ...
  static Map<String, List<String>> get featuresByCategory => {
    ...
    'Interactive': ['QR Scanner', 'AR Experiences', 'Map Gallery', 'Social Feed'],
    'Support': ['Feedback', 'Help', 'Settings'],
    'Functionality': ['Search', 'Filtering', 'Favorites', 'Notifications', 'Offline Mode', 'Caching'],
    'UI/UX': ['Animations', 'Dark Mode', 'Accessibility', 'Haptic Feedback'],
    'Development': ['Debug Mode', 'Debug Logging', 'Performance Monitoring', 'Mock Data'],
  ...
  static Map<String, List<String>> get enabledFeaturesByCategory {
    ...
    for (final entry in featuresByCategory.entries) {
      final enabled = entry.value.where((feature) => isEnabled(feature)).toList();
  ...
  static Map<String, List<String>> get disabledFeaturesByCategory {
    ...
    for (final entry in featuresByCategory.entries) {
      final disabled = entry.value.where((feature) => !isEnabled(feature)).toList();
  ...
  static List<String> get nonCriticalFeatures {
    ...
    return allFeatures.where((feature) => !criticalFeatures.contains(feature)).toList();
  }
  ...
  static List<String> validateConfiguration() {
    ...
    if (isEnabled('Mock Data') && isEnabled('API')) {
      issues.add('Both Mock Data and API are enabled - this may cause conflicts');
    }
    
    // Check for missing dependencies
    if (isEnabled('Streaming') && !isEnabled('Notifications')) {
      issues.add('Streaming is enabled but Notifications are disabled - users may miss live events');
    }
  ...
  static List<String> getRecommendations() {
    ...
    if (!isEnabled('Search')) {
      recommendations.add('Consider enabling Search for better user experience');
    }
    
    if (!isEnabled('Favorites')) {
      recommendations.add('Consider enabling Favorites to let users save events');
    }
    
    if (!isEnabled('Offline Mode')) {
      recommendations.add('Consider enabling Offline Mode for better connectivity handling');
    }
    
    if (!isEnabled('Dark Mode')) {
      recommendations.add('Consider enabling Dark Mode for better accessibility');
    }
